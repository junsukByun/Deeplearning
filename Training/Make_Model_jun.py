...
class LGD_non_equip_model(object):
    ...
    def __init__(self, train_path, test_path, tfrecord_filename, file_read_mode, network, learning_rate, batch_size, n_classes, pos_weight, target_leak_ratio, n_epochs, model_nm):
...
#         self.Nonkiller_batch_size = int(self.batch_size - self.Killer_batch_size)
    ...
    def get_data_under_construction_v2(self):          
        with tf.name_scope('get_data'):
            ...
            with tf.name_scope('Train_Killer'):
                ...
                self.iterator_Killer = self.train_Killer.make_one_shot_iterator()
            ...
            with tf.name_scope('Train_Nonkiller'):
                ...
                self.iterator_Nonkiller = self.train_Nonkiller.make_one_shot_iterator()
           
            with tf.name_scope('Test'):            
                self.test_dataset = self.test_dataset.batch(1000)
                self.test_dataset = self.test_dataset.prefetch(1)
                self.test_dataset = self.test_dataset.cache()
                self.iterator_test = self.test_dataset.make_one_shot_iterator()
    ...
    def get_data(self):
        with tf.name_scope('get_data'):
            train_dataset, test_dataset = read_train_and_test(self.tf_filenm) 
            self.train_Killer = train_dataset.filter(lambda x,y: tf.reshape(tf.cast(x[0], tf.bool), [])).repeat() #1만 [1,0]
            self.train_Nonkiller = train_dataset.filter(lambda x,y: tf.reshape(tf.cast(x[1], tf.bool), [])) #10만 [0,1]
            print(self.train_Nonkiller)
            self.balanced_dataset = tf.data.Dataset.zip((self.train_Killer, self.train_Nonkiller)) #1만 1만 = 2만 ([[1,0], [0,1]], [[1...]])
            self.balanced_dataset = self.balanced_dataset.flat_map(lambda ex_train_Killer, ex_train_Nonkiller: tf.data.Dataset.from_tensors(ex_train_Killer)
                                       .concatenate(tf.data.Dataset.from_tensors(ex_train_Nonkiller))) 
            self.balanced_dataset = self.balanced_dataset.shuffle(buffer_size=(int(self.n_train)*2 + 3 * self.batch_size))   # buffer size 임의 설정
            self.balanced_dataset = self.balanced_dataset.batch(self.batch_size)
            self.test_dataset = test_dataset.batch(self.batch_size)
            
            self.iterator = tf.data.Iterator.from_structure(self.balanced_dataset.output_types, self.balanced_dataset.output_shapes)

            self.labels, self.images = self.iterator.get_next()

            self.train_init = self.iterator.make_initializer(self.balanced_dataset)  # initializer for train_data
...
#         return step     
    ...
    def train_one_epoch(self, sess, saver, init, writer, epoch, step, save_path):
        ...
        try:
            while True:
                ...
                writer.add_summary(summaries, global_step=step)
                if step % self.skip_steps == 0:
                    print('Loss at epoch {0} (step: {1}): {2}'.format(epoch,step,l))
                step += 1
                total_loss += l
                n_batches += 1
        except tf.errors.OutOfRangeError:
        ...
        saver.save(sess, 'checkpoints/'+ save_path +'/' + save_path, step, write_meta_graph = True)        
#         print('Average loss at epoch {0} (step: {1}) : {2}'.format(epoch, step, total_loss/n_batches))
        self.train_one_time = time.time() - start_time
#         print('Took: {0} seconds'.format(self.train_one_time))
        return step
    ...
    def eval_once(self, sess, init, writer, epoch, step):
        ...
        try:
            while True:
                labels_batch, preds_batch, accuracy_batch, leak_batch, summaries = sess.run([self.labels, self.preds, self.accuracy, self.leak_ratio, self.summary_op], feed_dict = {self.is_training: False})
                writer.add_summary(summaries, global_step=step)
        ...
        with tf.Session(config= config) as sess:
            ...
            if ckpt and ckpt.model_checkpoint_path: ## 이미 모델이 만들어지고 재학습하는 경우
                ...
                with open('checkpoints/' + self.model_nm + '/Modeling_information.txt', 'a') as f:
                    ...
                    f.writelines('train_data : {}\n, valid_data : {}\n, used_TFrecord : {}\n, file_read_mode : {}\n, network: {}\n, learning_rate: {}\n, pos_weight : {}\n, target_leak_ratio : {}\n, batch_size : {}\n, epochs : {}\n'.format( self.train_path, self.test_path, self.tfrecord_filename, self.file_read_mode, self.network, self.lr, self.pos_weight, self.target_leak_ratio, self.batch_size, self.n_epochs))
            ...
            for epoch in range(self.n_epochs):
                ...
                step = self.train_one_epoch(sess, saver, self.train_init, writer, epoch, step, save_path) #self.train_init
                self.eval_once(sess, self.test_init, writer, epoch, step)# self.test_init
